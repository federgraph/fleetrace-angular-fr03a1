...
export class TMsgParser extends TLineParser {
  ...
  private IsTimeCommand(): boolean {
    return (
      this.sCommand === 'ST' || this.sCommand.substring(0, 2) === 'IT' || this.sCommand === 'FT'
    );
  }
  ...
  protected override ParseKeyValue(sKey: string, sValue: string): boolean {
    let result = false;
    this.Clear();
    this.FKey = sKey;
    this.FValue = sValue;
