import { Component, OnDestroy, Output, EventEmitter, Input, OnInit, inject } from '@angular/core';
...
export class ConnectionControlComponent implements OnInit, OnDestroy {
  @Input() wantInput = true;
  @Input() wantOutput = true;

  @Input() mode = 1;
  @Output() newNettoAvailable = new EventEmitter<string>();
  ...
  toggleWatchNetto() {
    if (this.subscription) {
      ...
      this.subscription = this.timingService.watchRace(-1).subscribe(
        (message) => {
  ...
  updateLEDColorInput(): void {
    switch (this.ledIn) {
      case AmpelColor.white:
        this.ledColorIn = 'white';
        break;
      case AmpelColor.red:
        this.ledColorIn = 'red';
        break;
      case AmpelColor.yellow:
        this.ledColorIn = 'yellow';
        break;
      case AmpelColor.green:
        this.ledColorIn = 'lime';
        break;
      default:
        this.ledColorIn = 'gray';
        break;
    }
  }
  ...
  updateLEDColorOutput(): void {
    switch (this.ledOut) {
      case AmpelColor.white:
        this.ledColorOut = 'white';
        break;
      case AmpelColor.red:
        this.ledColorOut = 'red';
        break;
      case AmpelColor.yellow:
        this.ledColorOut = 'yellow';
        break;
      case AmpelColor.green:
        this.ledColorOut = 'lime';
        break;
      default:
        this.ledColorOut = 'gray';
        break;
    }
  }
    ...
    if (this.subscription) {
      ...
      if (this.inputConnected) {
        this.apiService.sendMsg(msg).subscribe(
          ...
          (err) =>
            console.log('Cannot retrieve response. Error code: %s, URL: %s ', err.status, err.url),
        );
      }
    }
  }
