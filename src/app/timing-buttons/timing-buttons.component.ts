...
@Component({
  ...
  imports: [FormsModule, MaterialModule, TimingWidgetComponent, IconBarLegendComponent],
...
export class TimingButtonsComponent implements OnInit {
  ...
  @Output() updateAll = new EventEmitter<number>();
  ...
  options = ['time', 'dns', 'dnf', 'dsq', 'ok', 'erase'];
  ...
  onKeyTimeReceived(tm: ITimingParams) {
    this.onTimeReceived(tm);
    this.update();
  }

  onTimeReceived(tm: ITimingParams) {
  ...
  onTimeCancelled(tm: ITimingParams) {
    this.CurrentBib = tm.bib;
  }
