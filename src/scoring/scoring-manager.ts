...
export class TScoringManager {
  ...
  scoreRegatta(regatta: TRegatta, inRaces: TRaceList): void {
    ...
    try {
      ...
      if (!this.fScoringModel || !regatta || inRaces.length === 0 || regatta.Entries.length === 0) {
        if (TScoringManager.sTraceStatus) {
  ...
  private scoreSeries(
    ...
    if (divRaces.length > 0) {
      r = divRaces[0];
      if (r.HasFleets && TRegatta.IsInFinalPhase) {
  ...
  private scoreSeries2(
    ...
    const doties: TScoringTiebreaker = new TScoringTiebreaker(
      this.fScoringModel,
      divRaces,
      divPointsList,
      divSeriesPoints,
    ...
    for (let e = 0; e < divSeriesPoints.Count; e++) {
      ...
      const nextpoints: number =
        e + 1 < divSeriesPoints.Count ? divSeriesPoints[e + 1].Points : 99999999.0;
   ...
   */
  CalcAveragePoints(divRacePoints: TRacePointsList, throwoutIsIncluded: boolean): void {
    if (TScoringManager.sTraceStatus) {
    ...
    eWithAvg.forEach((e: TEntry) => {
      ...
      const tempPts: number[] = new Array<number>(list.Count);
      const tempPen: number[] = new Array<number>(list.Count);
      let t = 0;
      ...
      list.forEach((p: TRacePoints) => {
        ...
        if (
          (!p.IsThrowout || throwoutIsIncluded) &&
          finish != null &&
          !finish.Penalty.hasPenalty(Constants.AVG)
        ) {
          pts = pts + p.Points;
          n++;
