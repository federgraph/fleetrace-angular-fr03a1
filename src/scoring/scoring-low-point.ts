...
export class TScoringLowPoint implements IScoringModel {
  ...
  protected setTiedPoints(value: TRacePoints[]): void {
    let pts = 0;
  ...
  protected scoreRace1(
    ...
    let pts: number = firstIs75 ? 0.75 : 1.0;
  ...
  calculateTieBreakersDefault(
    ...
    const eLists: TRacePointsList[] = new Array<TRacePointsList>();
    entries.forEach((e: TEntry) => {
      const ePoints: TRacePointsList = points.findAllPointsForEntry(e);
      eLists.push(ePoints);
    });

    const tiedWithBest: TRacePointsList[] = new Array<TRacePointsList>();
  ...
  calculateTieBreakersAlternate(
    ...
    const rpLists: TRacePointsList[] = new Array<TRacePointsList>();
    entries.forEach((e: TEntry) => {
   ...
   */
  protected comparePointsBestToWorst(inLeft: TRacePointsList, inRight: TRacePointsList): number {
   ...
   */
  protected compareWhoBeatWhoLast(stillTied: TRacePointsList[], series: TSeriesPointsList): void {
    ...
    const beatenCount: number[] = new Array<number>(nTied);
      ...
      for (let o = 0; o < nTied; o++) {
        if (e !== o && beatenCount[e] === beatenCount[o]) {
          ...
          for (let r = nRaces - 1; r >= 0; r--) {
            ...
            if (ePts > oPts) {
              this.incrementSeriesScore(
                tiedEntries[e],
                TScoringLowPoint.TIEBREAK_INCREMENT,
                series,
              );
            }
            if (ePts !== oPts) {
  ...
  protected incrementSeriesScore(e: TEntry, amount: number, series: TSeriesPointsList): void {
  ...
  getPenaltyPoints(p: TRSPenalty, rpList: TRacePointsList, basePts: number): number {
    ...
    if (
      p.hasPenalty(Constants.STP) ||
      p.hasPenalty(Constants.MAN) ||
      p.hasPenalty(Constants.RDG) ||
      p.hasPenalty(Constants.DPI)
    ) {
      return p.Points;
    }
    ...
    if (p.isDsqPenalty()) {
      ...
      return this.IsLongSeries
        ? nEntries - rpList.getNumberWithPenalty(Constants.DNC) + 1
        : nEntries + 1;
    }
  ...
  calcThrowouts(pointsList: TRacePointsList): void {
    ...
    for (let i = 0; i < nThrows; i++) {
      ...
      pointsList.forEach((thisRace: TRacePoints) => {
        ...
        if (!b) {
          if (
            !thisRace.Finish.Penalty.hasPenalty(Constants.DNE) &&
            !thisRace.Finish.Penalty.hasPenalty(Constants.DGM) &&
            !thisRace.Finish.Penalty.hasPenalty(Constants.AVG)
          ) {
            if (!thisRace.IsThrowout && (worstRace == null || thisRace.Points > worstRace.Points)) {
              worstRace = thisRace;
            }
          }
        }
