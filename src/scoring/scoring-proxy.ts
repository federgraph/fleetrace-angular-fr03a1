...
export class TRaceInfo extends TProxyProps {
  // Input
  OTime = 0; // int; Original Time/FinishPosition
  QU = 0; // int; Encoded Penalty Type ('QUit Packet')
  PenaltyPoints = 0.0; // double
  PenaltyNote = '';
  PenaltyPercent = 0; // int
  PenaltyTimePenalty = 0; // long

  // Output
  CPoints = 0.0; // double; Calculated Points
  Rank = 0; // int
  PosR = 0; // int; unique/eindeutiges Ranking
  PLZ = 0; // int; PlatzZiffer
  Drop = false; // IsThrowout
  Fleet = 0; // int
  IsRacing = true;
...
export class TEntryInfo extends TProxyProps {
  Index = 0; // int; Index in EntryInfoCollection
  SNR = 0; // int; Primary Key (SportlerNummer/SailNumber)
  IsGezeitet = true;
  IsTied = false;
  RaceList: TRaceInfo[];
