...
export class TOTimeErrorList {
  private FL: TEventRowCollectionItem[];
  ...
  private CheckOTimeForFleet(cl: TEventRowCollectionItem[], r: number): boolean {
  ...
  private AddContiguousErrorA(cl: TEventRowCollectionItem[], r: number, position: number): void {
    let cr: TEventRowCollectionItem;
    let re: TEventRaceEntry;
    for (cr of cl) {
      re = cr.Race[r];
  ...
  protected AddFinishError(r: number, cr: TEventRowCollectionItem, e: TFinishError): void {
    let s: string =
      'Error.' + this.BO.MsgToken.cTokenSport + this.BO.MsgToken.cTokenRace + r.toString();
    if (cr != null) {
      s = s + '.ID' + cr.BaseID.toString();
    }
  ...
  private IsPreconditionForFleet(cl: TEventRowCollectionItem[], r: number): boolean {
    // let a: number[]; // array of Integer;
    let cr: TEventRowCollectionItem;
    ...
    const EntryCount = cl.length;
    const a = new Array<number>(EntryCount + 1); // SetLength(a, EntryCount + 1);
