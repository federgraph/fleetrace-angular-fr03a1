...
import { TNameValueRowCollection, TNameValueRowCollectionItem, NameValueFieldType } from "../col/name-value/name-value-grid";
...
export class TScoringSystemStruct {
    ...
    static getComboStrings(): Array<string> {
        const sa: string[] = new Array(TScoringSystemStruct.ScoringSystemHigh + 1);
        for (let i = TScoringSystemStruct.ScoringSystemLow; i <= TScoringSystemStruct.ScoringSystemHigh; i++) {
...
export class TEventProps extends TLineParser {
    ...
    // Uniqua Props
    ShowCupColumn = false;
    EnableUniquaProps: boolean = false; // override calculated values
    ...
    get RaceLayout(): string {
        if (this.BO.EventNode.ShowPoints === TEventNode.Layout_Finish)
            return "Finish";
        else
    ...
    set RaceLayout(value: string) {
        if (value === "Finish")
            this.BO.EventNode.ShowPoints = TEventNode.Layout_Finish;
        else
            this.BO.EventNode.ShowPoints = TEventNode.Layout_Points;
    }
    ...
    set NameSchema(value: string) {
        switch (value) {
            case "NX": FieldNames.setSchemaCode(2); break;
            case "LongNames": FieldNames.setSchemaCode(1); break;
            default: FieldNames.setSchemaCode(0); break;
        }
    }
